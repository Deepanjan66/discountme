...
class PostManager:
    ...
    def get_post_by_user(id):
        data = dao.read(USER_POSTS)
        data = [Discount_post(row) for row in data if USER_POSTS['uid'] == id]
        post_obj = dao.read(POSTS)
        post_obj = [Discount_post(row) for row in post_obj]
        post = []
        for post_id in data:
            for obj in post_obj:
                if POSTS['id'] == post_id:
